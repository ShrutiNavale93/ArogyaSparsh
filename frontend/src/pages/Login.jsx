...
// ⚠️ NOTE: I have commented out your local images to make this code run here.
// Please UNCOMMENT these two lines in your local project:
import logo from '../assets/logo_final.png';
import loginBg from '../assets/medicines/loginimg.png';

// Temporary placeholders for preview:
const logo = "https://via.placeholder.com/150x50?text=ArogyaSparsh"; 
const loginBg = "https://images.unsplash.com/photo-1584982751601-97dcc096654c?auto=format&fit=crop&q=80&w=1000";
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    //  ✅ SMART URL (Localhost or Cloud)
    // We use a safe check for import.meta to prevent build warnings in some environments
    const apiKey = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_API_URL) || "http://localhost:5000";
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/auth/login`, { 
        ...
        body: JSON.stringify({ email, password }) // Sends strict credentials
      });
      
      const data = await res.json();
      
      if (res.ok) {
        //  🎉 SUCCESS: Database Login Worked
        console.log("Login Successful:", data);
        localStorage.setItem('userInfo', JSON.stringify(data));
        redirectUser(data.role);
      } else {
        //  ❌ ERROR: Database rejected it (Wrong password/email)
        setError(data.message || "Invalid email or password");
      }
    } catch (err) {
      console.error("Network Error:", err);
      setError("Server is offline. Make sure 'node server.js' is running.");
    }
    setLoading(false);
  };
  ...
  //  ✅ Helper to get correct placeholder based on selected tab
  const getPlaceholder = () => {
      if (role === 'phc') return "phc.chamorshi@arogya.com";
      if (role === 'hospital') return "chamorshi@hospital.com";
      return "admin@arogya.com";
  };
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                {role === 'phc' ? 'Official Email' : 'Official Email'}
            </label>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12 text-white z-10">
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-2xl shadow-2xl max-w-lg animate-in slide-in-from-bottom-10 duration-1000">
            <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs font-bold tracking-widest text-green-400 uppercase">System Online • Database Connected</span>
            </div>
